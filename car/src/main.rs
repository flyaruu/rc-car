...
use hal::{
    ...
    interrupt::enable,
    ledc::{channel::config::PinConfig, timer, LSGlobalClkSource, Ledc, LowSpeed},
...
use log::info;
use protocol::{
    ControlMessage, Message, MessageChannel, MessagePublisher, MessageSubscriber, TelemetryMessage,
};
...
use crate::{
    blinkers::blinker,
    lights::{
        brakelight_controller, brakelight_motor_monitor, light_controller, reverselight_controller,
        reverselight_motor_monitor, HeadlightController,
    },
    net::{receiver, sender},
    servo::Servo,
    types::{
        MotorServo, SteeringServo, HEADLIGHT_CHANNEL, LED_TIMER_NUMBER, MOTOR_CHANNEL,
        MOTOR_TIMER_NUMBER, SERVO_TIMER_NUMBER, STEERING_CHANNEL,
    },
};
...
fn main() -> ! {
    ...
    let steering_servo: &'static mut SteeringServo =
        Box::leak(Box::new(Servo::new(steering_channel)));
    ...
    executor.run(|spawner| {
        ...
        spawner
            ...
            .unwrap();
        spawner
            .spawn(heartbeat(command_channel.publisher().unwrap()))
            .unwrap();
        spawner
            .spawn(blinker(
                spawner,
            ...
            .unwrap();
        spawner
            .spawn(light_controller(
                command_channel.subscriber().unwrap(),
                headlight_controller,
            ))
            .unwrap();
        spawner
            .spawn(reverselight_controller(
                command_channel.subscriber().unwrap(),
                reverselight_pin,
            ))
            .unwrap();
        spawner
            .spawn(reverselight_motor_monitor(
                command_channel.subscriber().unwrap(),
                command_channel.publisher().unwrap(),
            ))
            .unwrap();
        spawner
            .spawn(brakelight_controller(
                command_channel.subscriber().unwrap(),
                brakelight_pin,
            ))
            .unwrap();
        spawner
            .spawn(brakelight_motor_monitor(
                command_channel.subscriber().unwrap(),
                command_channel.publisher().unwrap(),
                rtc,
            ))
            .unwrap();
        spawner
            .spawn(test_lights(command_channel.publisher().unwrap()))
            .unwrap();
        spawner
            .spawn(tach::tach(
                spawner,
...
async fn steering(
    ...
    steering_servo: &'static mut SteeringServo,
) -> ! {
...
async fn motor(
    mut subscriber: MessageSubscriber,
    motor_servo: &'static mut MotorServo,
    ...
    loop {
        match subscriber.next_message_pure().await {
            Message::Control(ControlMessage::MotorPower(value)) => {
                ...
                publisher
                    .publish(Message::Telemetry(TelemetryMessage::MotorSetting(duty)))
                    .await;
            }
...
async fn test_lights(publisher: MessagePublisher) -> ! {
    ...
    publisher
        .publish(Message::Control(ControlMessage::BlinkerCommand(
            protocol::BlinkerState::Alarm,
        )))
        .await;
    Timer::after_millis(2500).await;
    publisher
        .publish(Message::Control(ControlMessage::BlinkerCommand(
            protocol::BlinkerState::Off,
        )))
        .await;

    publisher
        .publish(Message::Control(ControlMessage::ReverselightCommand(
            protocol::ReverseLights::On,
        )))
        .await;
    publisher
