...
pub async fn tach(
    ...
    spawner
        .spawn(revolution(tach_pin, rtc, odo_signal, rpm_signal))
        .unwrap();
    spawner
        .spawn(tach_publisher(publisher, odo_signal, rpm_signal, rtc))
        .unwrap();
}

#[task]
pub async fn revolution(
    mut tach_pin: AnyInput<'static>,
    rtc: &'static Rtc<'static>,
    odo_signal: &'static Signal<NoopRawMutex, u64>,
    rpm_signal: &'static Signal<NoopRawMutex, u64>,
) -> ! {
    let mut odo = 0_u64;
...
pub async fn tach_publisher(
    ...
    loop {
        if odo_signal.signaled() {
            ...
            publisher
                .publish(protocol::Message::Telemetry(
        ...
        if rtc.get_time_us() - last_signal > 1000000 {
            publisher
                .publish(protocol::Message::Telemetry(
                    protocol::TelemetryMessage::MotorRpm(0),
                ))
                .await;
            last_signal = rtc.get_time_us();
        }
